    ...
    case queue:out(Queue) of
        ...
        {{value, ReqId}, Queue2} ->
            case maps:take(ReqId, Map) of
                error -> {ok, Channel, State#state{inflight_queue = Queue2}};
                {Inflight, Map2} ->
                    ...
                    case produce_chunk(State2, Channel, Conn,
                                       MsgType, final, ReqId, Data) of
                        {error, _Reason, _Channel2, _State3} = Error -> Error;
                        {ok, Chunk, Channel2, State3} ->
                            Output = opcua_uacp_codec:encode_chunks(Chunk),
                            {ok, Output, Channel2, State3}
                    end
            end
    end.
