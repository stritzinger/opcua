...
decode_schema(#opcua_structure{with_options = false, fields = Fields}, Data) ->
    decode_fields(Fields, Data);
decode_schema(#opcua_structure{with_options = true, fields = Fields}, Data) ->
    {Mask, Data1} = decode_builtin(uint32, Data),
    decode_masked_fields(Mask, Fields, Data1);
decode_schema(#opcua_union{fields = Fields}, Data) ->
...
decode_fields([Field | Fields], Data, Acc) ->
    ...
    Acc1 = maps:put(Field#opcua_field.name, Value, Acc),
    decode_fields(Fields, Data1, Acc1).

decode_field(#opcua_field{node_id = NodeId, value_rank = -1}, Data) ->
    decode_type(NodeId, Data);
decode_field(#opcua_field{node_id = NodeId, value_rank = N}, Data) when N >= 1 ->
...
decode_extension_object(Data) ->
    ...
    case decode_extension_object(Mask, TypeId, T) of
        ...
        {#opcua_extension_object{type_id = NodeSpec, encoding = byte_string, body = Body} = ExtObj, Data2} ->
        ...
        {#opcua_extension_object{}, _Data2} ->
            throw(bad_data_encoding_unsupported)
    end.

decode_extension_object(16#00, TypeId, T) ->
    {#opcua_extension_object{type_id = TypeId, encoding = undefined, body = undefined}, T};
decode_extension_object(16#01, TypeId, T) ->
    {Body, T1} = decode_builtin(byte_string, T),
    {#opcua_extension_object{type_id = TypeId, encoding = byte_string, body = Body}, T1};
decode_extension_object(16#02, TypeId, T) ->
    {Body, T1} = decode_builtin(xml, T),
    {#opcua_extension_object{type_id = TypeId, encoding = xml, body = Body}, T1};
...
encode_schema(#opcua_structure{with_options = false, fields = Fields}, Data) ->
    encode_fields(Fields, Data);
encode_schema(#opcua_structure{with_options = true, fields = Fields}, Data) ->
    encode_masked_fields(Fields, Data);
encode_schema(#opcua_union{fields = Fields}, UnionMap) ->
    [Name] = maps:keys(UnionMap),
    [Field] = [Field || Field = #opcua_field{name=FieldName} <- Fields, FieldName==Name],
    SwitchValue = encode_builtin(uint32, Field#opcua_field.value),
...
encode_masked_fields([Field = #opcua_field{is_optional = false, name = Name} | Fields], Data, Mask, Acc) ->
...
encode_masked_fields([Field = #opcua_field{is_optional = true, name = Name} | Fields], Data, Mask, Acc) ->
    case maps:take(Name, Data) of
        {Value, Data2} ->
            {EncodedField, _} = encode_field(Field, Value),
            encode_masked_fields(Fields, Data2, [Field#opcua_field.value|Mask], [EncodedField|Acc]);
        error ->
...
encode_fields([Field | Fields], Data, Acc) ->
    {_, {Value, Data2}} = {Field#opcua_field.name, maps:take(Field#opcua_field.name, Data)},
    {FieldValue, _} = encode_field(Field, Value),
    encode_fields(Fields, Data2, [FieldValue|Acc]).

encode_field(#opcua_field{node_id = NodeId, value_rank = -1}, Data) ->
    encode_type(NodeId, Data);
encode_field(#opcua_field{node_id = NodeId, value_rank = N}, Array)
...
encode_extension_object(#opcua_extension_object{type_id = NodeSpec,
...
encode_extension_object(#opcua_extension_object{}) ->
    throw(bad_data_encoding_unsupported).
...
encode_variant(#opcua_variant{type = Type, value = Value}) ->
    TypeId = opcua_codec:builtin_type_id(Type),
